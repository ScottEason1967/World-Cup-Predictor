...
CURRENT_ROUND = "final"          # group | r32 | r16 | qf | sf | final
...
# 3 -- round switcher lists every open round ----------------------------------
print("\n3. Round switcher links all open rounds")
switch_pages = [f for f in tabs_pages if os.path.basename(f).replace(".html","") in open_rounds] + ["index.html"]
for f in sorted(set(switch_pages)):
    s = read(f)
    for r in open_rounds:
        check(f'href="{r}.html"' in s, f"{f}: links to {r.upper()}")
